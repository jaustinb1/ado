...
class GraphADO(ADO):
    ...
    def __custom_dijkstras(self,
        ...
        distances[source] = 0
        pq_push(frontier, source, distances[source])
        ...
        while heap_nodes:
            ...
            if u in seen:
                continue
            seen.add(u)
            for v in graph.iterNeighbors(u.nx_node):
                v = self._nx_node_to_vertex[v]
                if v in seen:
                    continue
                dist_through_u = distances[u] + self.g.weight(u.nx_node,v.nx_node) #self.distance(u, v)
    ...
    def compute_p(self):
        ...
        for i in range(self.k - 1, -1, -1):
            ...
            for v in self.A[i]:
                self.g.addEdge(v.nx_node, n, w=0.0)
            ...
            self.g.removeNode(n)  # Remove temporary node
            print("Network", len(self.differences[i]), i, time.time() - start)
    ...
    def __plot_query_state(self,
        ...
        #pos = {(x, y): (y, -x) for x, y in self.g.iterNodes()}
        #nx.draw(self.g,
        #        pos=pos,
        #        ax=ax,
        #        node_size=100,
...
def generate_random_weighted_2d_grid_graph(m: int = 5, n: int = 5,
    ...
    #erg = nk.generators.ErdosRenyiGenerator(n, 0.2)
    #return erg.generate()
...
if __name__ == "__main__":
    ...
    print(query_points)
    print(graph_ado.query(query_points[0], query_points[1]))
